...
<cfset room = application.apartment.getRoomById(room_id="#url.room_id#") />
<cfset films = room.getFilmsByRoomId()/>
	...
	<table class="myborder" style="font-family:Arial, Helvetica, sans-serif;" cellspacing="0px;" width="100%">
		...
		<cfloop index="film" array="#films#">
			<tr <cfif count % 2>class="background"</cfif>>
				...
				<td>
					<a href="javascript:if(confirm('Are you sure you want to delete #film.getTitle()#?')){window.location='#application.ComponentRoot#services/FilmService.cfc?method=deleteFilm&film_id=#film.getId()#&room_id=#room.getID()#';}"><img src="#application.imagePrefix#delete.gif" style="border:none;"/></a>
				</td>
			</tr>
			<cfset count += 1/>
		</cfloop>
